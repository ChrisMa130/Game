...
public class LineManagement : MonoBehaviour 
{
	...
	private bool validLine;
		...
		private List<GameObject> forbidden;
	private GameObject controller;

	void Awake() {
		forbidden = new List<GameObject> ();
		Colliders = GameObject.FindGameObjectWithTag ("Colliders");
	    ...
	    private void createLine()
	    {
				...
				line.GetComponent<Renderer>().sortingLayerName = "Foreground";
		    }
		...
		/*
				// now add the check collider
		GameObject boxCheck = new GameObject();
		boxCheck.transform.position = col.transform.position;
		boxCheck.transform.parent = GameObject.FindGameObjectWithTag ("Colliders").transform;
		BoxCollider2D check = boxCheck.gameObject.AddComponent<BoxCollider2D> ();
		check.isTrigger = true;
		float checkLength = lineLength + 0.1f * lineLength;
		check.transform.Rotate (0, 0, angle);
		check.size = new Vector2 (checkLength, 0.06f);
		check.transform.localPosition = midPoint;
		check.offset = new Vector2 (0, 0);
		boxCheck.AddComponent <ForbiddenZone>();
		*/
	}
}